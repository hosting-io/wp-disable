...
class Optimisationio_Dashboard {
	...
	function __construct() {
		...
		if ( class_exists( 'Folium_UI' ) ) {
			// Nest WP Disable under the single shared "Folium" admin menu and open
			// in-frame at admin.php?page=wp-disable. No own top-level item.
			Folium_UI::register_plugin( array(
				'slug'     => 'wp-disable',
				'name'     => __( 'WP Disable', 'wp-disable' ),
				'tagline'  => __( 'Bloat & request removal', 'wp-disable' ),
				'icon'     => 'D',
				'icon_url' => plugin_dir_url( dirname( __FILE__ ) ) . 'images/wp-disable.jpg',
				'render'   => array( $this, 'dashboard_page' ),
			) );
			...
			// Degraded fallback (folium-ui is vendored, so this should not happen):
			// keep the original standalone top-level menu.
	...
	public function statistics_menu() {
		add_menu_page( __( 'Folium Studio', 'wp-disable' ), __( 'Folium Studio', 'wp-disable' ), 'manage_options', 'optimisationio-dashboard', array( $this, 'dashboard_page' ), 'dashicons-dashboard' );
	}
	...
	/**
	 * Enqueue the WP Disable Folium app (JS + CSS) and inject real settings when
	 * the shared frame renders the wp-disable screen. Hooked to folium_ui_enqueue.
	 *
	...
	public function on_folium_enqueue( $slug ) {
		...
		$css_ver = file_exists( $css ) ? (string) filemtime( $css ) : ( defined( 'WP_DISABLE_VERSION' ) ? WP_DISABLE_VERSION : '2.1.0' );
		$js_ver  = file_exists( $js )  ? (string) filemtime( $js )  : ( defined( 'WP_DISABLE_VERSION' ) ? WP_DISABLE_VERSION : '2.1.0' );
	...
	/**
	 * Real data injected into the WP Disable Folium app.
	 *
	 * @return array
	 */
	...
	private static function display_stats__cache_and_database_and_wp_disable(){
		...
		<div class="addon-stats">
			<ul class="cache-and-database-list">
				<li><?php esc_html_e( 'Original DB', 'wp-disable' ); ?><span><?php Optimisationio_Dashboard::echo_stats_size( $active_addon, $active_addon ? $cache_info->size : 0 ); ?></span></li>
				<li><?php esc_html_e( 'New DB', 'wp-disable' ); ?><span><?php Optimisationio_Dashboard::echo_stats_size( $active_addon, $active_addon ? $cache_info->optimised_size : 0 ); ?></span></li>
				<li><?php esc_html_e( 'Savings', 'wp-disable' ); ?><span><?php Optimisationio_Dashboard::echo_stats_size( $active_addon, $active_addon ? $cache_info->saving : 0 ); ?></span></li>
			</ul>
			<ul class="cache-and-database-list">
				<li><?php esc_html_e('Pages average load time', 'wp-disable'); ?><span><?php echo $active_addon ? Optimisationio::average_pages_load_time() : '<i class="n_a">' . self::$str_i18n['n/a'] . '</i>'; ?></span></li>
			</ul>
			<ul class="cache-and-database-list">
				<li><?php esc_html_e( 'Cache', 'wp-disable' ); ?><span><?php Optimisationio_Dashboard::echo_stats_size( $active_addon, $active_addon ? Optimisationio_CacheEnabler::get_cache_size() : 0 ); ?></span></li>
